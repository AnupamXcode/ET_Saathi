...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", isLoading, children, ...props }, ref) => {
    ...
    const variants = {
      default: "bg-primary text-primary-foreground hover:bg-primary/90 shadow-[0_0_15px_rgba(200,150,50,0.15)] hover:shadow-[0_0_20px_rgba(200,150,50,0.3)] border border-primary/20",
      outline: "border border-input bg-transparent hover:bg-white/5 text-foreground hover:text-primary",
      ghost: "hover:bg-white/5 hover:text-primary text-foreground",
      link: "text-primary underline-offset-4 hover:underline",
      destructive: "bg-destructive/10 text-destructive border border-destructive/20 hover:bg-destructive/20",
      positive: "bg-positive/10 text-positive border border-positive/20 hover:bg-positive/20",
    }
